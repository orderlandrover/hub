...
export default function App() {
  ...
  async function load(over?: Partial<{ page: number; status: string; search: string }>) {
    ...
    try {
      const q = new URLSearchParams({ page: String(p), status: st, search: se });
      const res = await fetch(`/api/products-list?${q.toString()}`, { signal: ctrl.signal });
      if (!res.ok) throw new Error(await res.text());
      const json = (await res.json()) as ListResponse;
      setData(json);
      setSelected([]); // rensa selektion vid ny datamängd
    } catch (e: any) {
  ...
  // Auto-load när sida eller status ändras (sök körs via knappen)
  useEffect(() => {
    load();
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-4 mb-6">
          <div className="flex flex-col md:flex-row gap-3 md:items-end">
            ...
            <div className="flex gap-2">
              {/* Viktigt: sätt sidan till 1 OCH fetcha med override så vi inte dubbelkallar */}
              <button
                onClick={() => { setPage(1); load({ page: 1, search }); }}
              ...
              <button
                onClick={() => { setSearch(""); setStatus("any"); setPage(1); load({ page: 1, status: "any", search: "" }); }}
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-4 mb-3 flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-3 py-2 rounded-xl bg-emerald-600 disabled:opacity-50 text-white"
          >
            Publicera
          </button>
          <button
            ...
            className="px-3 py-2 rounded-xl bg-amber-600 disabled:opacity-50 text-white"
          >
            Avpublicera
          </button>
        ...
        <div className="overflow-auto bg-white rounded-2xl shadow-sm border">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading && <tr><td colSpan={7} className="p-6 text-center">Laddar…</td></tr>}
              {!loading && data.items.length === 0 && <tr><td colSpan={7} className="p-6 text-center">Inga produkter</td></tr>}
