...
const Chip = "inline-flex items-center rounded-full border px-2 py-0.5 text-xs capitalize";

/* ---------- Base64 helper (säker för stora filer) ---------- */
function fileToBase64(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    const fr = new FileReader();
    fr.onerror = () => reject(fr.error || new Error("Fil-läsning misslyckades"));
    fr.onload = () => {
      const res = String(fr.result || "");
      // data:...;base64,XXXX  -> vi vill bara åt själva base64-delen
...
/* =====================================================================
   Flik 1 – Produkter (oförändrat förutom klassnamn)
===================================================================== */
function ProductsTab() {
  ...
  return (
    <>
      ...
      {/* Bulk actions */}
      <section className={`${Card} p-4 mb-4 flex flex-wrap items-center gap-3`}>
        ...
        <button disabled={selected.length===0 || loading} onClick={async()=>await bulkDelete()} className="ml-auto px-4 py-2 rounded-lg ui-btn ui-btn--danger">Radera</button>
...
/* =====================================================================
   Flik 2 – Import & synk (prisfil + FX/påslag/avrundning + dry-run)
===================================================================== */
function ImportTab() {
  ...
  // (behåll subkategorier vid behov)
  ...
  async function handlePriceUpload(file: File) {
    try {
      ...
      // Mappa UI → API
      const apiRound = roundMode === "nearest" ? "near" : roundMode;     // backend förstår "near"
      const stepToSend = roundMode === "none" ? 0 : Number(roundStep) || 0;
      ...
      const res = await fetch("/api/price-upload", {
        ...
        body: JSON.stringify({
          ...
          roundMode: apiRound,   // "near" | "up" | "down" | "none"
          step: stepToSend,
          publish: pub,
          dryRun: !!dry,
        }),
      ...
      let j: any = null;
      try { j = txt ? JSON.parse(txt) : null; } catch { /* icke-JSON */ }

      if (!res.ok) {
        const msg = (j && j.error) ? j.error : `HTTP ${res.status} – ${txt?.slice(0, 200)}`;
        throw new Error(msg || "Fel vid prisimport");
      }

      addLog(`OK: total=${j.total}, updated=${j.updated}, skipped=${j.skipped}, notFound=${j.notFound}, errors=${j.errors}`);
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6">
      ...
      <section className={`${Card} p-5`}>
        ...
        <p className="text-sm opacity-70 mb-3">Matchar på <b>SKU</b>. Pris = GBP × valutakurs × (1 + påslag%) → avrundning.</p>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
          <div>
            <label className="text-xs opacity-70">Valutakurs (GBP→SEK)</label>
            <input type="number" step="0.01" className="w-full rounded-lg border px-3 py-2" value={fx} onChange={e=>setFx(Number(e.target.value)||0)} />
          </div>
          <div>
            <label className="text-xs opacity-70">Påslag (%)</label>
            <input type="number" step="0.1" className="w-full rounded-lg border px-3 py-2" value={markup} onChange={e=>setMarkup(Number(e.target.value)||0)} />
          </div>
          <div>
            <label className="text-xs opacity-70">Avrundning</label>
            <select className="w-full rounded-lg border px-3 py-2" value={roundMode} onChange={e=>setRoundMode(e.target.value as any)}>
          ...
          <div>
            ...
            <select className="w-full rounded-lg border px-3 py-2" value={roundStep} onChange={e=>setRoundStep(Number(e.target.value))} disabled={roundMode==="none"}>
      ...
      {/* C: Underkategorier (placeholder) */}
      <section className={`${Card} p-5`}>
        <h2 className="text-lg font-semibold mb-1">Britpart underkategorier</h2>
        <p className="text-sm opacity-70 mb-3">Välj en eller flera och kör dry-run/import (kommer användas i nästa steg).</p>
        <div className="h-48 overflow-auto rounded-lg border p-2 bg-white">
          {bpSubs.map((s) => (
            <label key={s.id} className="flex items-center gap-2 py-1">
              <input type="checkbox" className="accent-amber-600" checked={selectedSubs.includes(s.id)} onChange={() => setSelectedSubs((prev) => prev.includes(s.id) ? prev.filter(x=>x!==s.id) : [...prev, s.id])} />
