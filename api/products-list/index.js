...
functions_1.app.http("products-list", {
    methods: ["GET"],
    authLevel: "anonymous",
    handler: async (req, ctx) => {
        try {
            ...
            if (search)
                baseQS.set("search", search);
            if (category)
                baseQS.set("category", category);
            if (!all) {
                baseQS.set("page", String(pageParam));
                const res = await (0, wc_1.wcRequest)(`/products?${baseQS.toString()}`);
            ...
            firstQS.set("page", "1");
            const firstRes = await (0, wc_1.wcRequest)(`/products?${firstQS.toString()}`);
            ...
            for (let p = 2; p <= maxPages; p++) {
                ...
                const r = await (0, wc_1.wcRequest)(`/products?${qs.toString()}`);
