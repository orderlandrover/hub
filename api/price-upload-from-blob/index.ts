...
type Body = {
  ...
  // Batchning inom intervallet
  batchSize?: number;     // hur många rader per inre batch (default 1000)

  // Intervall för den här HTTP-körningen
  offset?: number;        // startindex i filen
  limitRows?: number;     // max rader att köra i den här körningen (default 2500)
};
...
app.http("price-upload-from-blob", {
  ...
  handler: async (req: HttpRequest, ctx: InvocationContext): Promise<HttpResponseInit> => {
    ...
    try {
      ...
      // Små diagnostik-lägen för felsökning
      if (diag === "peek") {
        const text = await downloadBlobText(body, ctx);
        return { status: 200, headers: CORS, jsonBody: { ok: true, length: text.length, head: text.slice(0, 600) } };
      }
      if (diag === "parse") {
        const text = await downloadBlobText(body, ctx);
        ...
        return {
          ...
          jsonBody: { ok: true, delimiter: ",", headers, sample: rows.slice(0, 10), total: rows.length }
        };
      }

      /* ----------- 1) Läs CSV från blob (konto-nyckel eller SAS) ----------- */
      const csvText = await downloadBlobText(body, ctx);

      /* ---------------------- 2) Parse CSV till rader ---------------------- */
      ...
      // Kör bara en delmängd per request (default 2 500 rader)
      const DEFAULT_LIMIT = 2500;
      const offset = Math.max(0, Math.min(total, Number((body.offset ?? 0))));
      const limitRows = Math.max(1, Math.min(total - offset, Number((body.limitRows ?? DEFAULT_LIMIT))));
      const end = Math.min(total, offset + limitRows);

      // Inre batch-storlek: hur många rader per "liten loop"
      const batchSize = Math.max(100, Math.min(5000, Number(body.batchSize ?? 1000)));

      /* -------------------- 4) Loop & uppdateringar ----------------------- */
      ...
      for (let off = offset; off < end; off += batchSize) {
        const slice = rows.slice(off, Math.min(off + batchSize, end));
        ...
        for (const raw of slice) {
          try {
            ...
            // --- Gör dry-run supersnabb: hoppa Woo-anrop ---
            if (dryRun) {
              updated++;
              if (sample.updates.length < 5) sample.updates.push({ sku, to: targetSEK, dryRun: true });
              continue;
            }

            // Hämta produkt på SKU
        ...
        processed = Math.min(off + batchSize, end);
        ctx.log?.(`[PRICE-BLOB] progress ${processed - offset}/${limitRows} (global ${processed}/${total}) updated=${updated} skipped=${skipped} notFound=${notFound} bad=${badRows}`);
      }
...
async function downloadBlobText(body: Body, ctx: InvocationContext): Promise<string> {
  // container+blobName via account key (privat)
  ...
  // Direkt-URL (sasUrl eller blobUrl)
